...
class Dynamic extends Controller {
    ...
    public function showDynamic() {
        ...
        /** -------------------------------------------------------------------------
         * FIX - 4 May 2023
         * On some server the webforms where causing an error (blocked:mixed-content)
         * This is because the webform dynamic url was loading on http:// even though
         * the site is running on https://
         * To fix this, the user can set ENFORCE_SSL = true and the code below
         * will form a change to the dynamic url to use https://
         * -------------------------------------------------------------------------*/
        if (env('ENFORCE_SSL')) {
            $page['dynamic_url'] = str_replace('http://', 'https://', $page['dynamic_url']);
        }
